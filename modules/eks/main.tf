...
resource "aws_security_group_rule" "cluster_ingress_bastion" {
  ...
  description              = "Allow bastion/VPN access to API server"
}
...
################################################################################
# Managed Node Group (Karpenter Bootstrap)
################################################################################
...
resource "aws_eks_node_group" "karpenter_bootstrap" {
  ...
  scaling_config {
    ...
    max_size     = var.bootstrap_node_count + 2
  ...
  labels = {
    "node.kubernetes.io/purpose" = "karpenter-bootstrap"
  }
